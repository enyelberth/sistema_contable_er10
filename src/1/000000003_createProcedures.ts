...
create or replace procedure "estate".sp_create_movement(
_administrative_unit_code character varying,
_motion_concept_code character varying,
_details jsonb,
_user_code character varying, out _code character varying)
  language plpgsql
  as $$
declare
  ...
  begin
    perform set_config('glb.user_current',_user_code,false);
    insert into "estate".movement(administrative_unit_code,motion_concept_code,obsevation) values (_administrative_unit_code,_motion_concept_code,'') returning code into _code;
  ...
  select count(*) into details_count from jsonb_array_elements(_details::jsonb);
   if details_count > 0 and _motion_concept_code = "02" then
      insert into "estate".movement_details(movement_code,group_code,sub_group_code,section,description,amount)
      select  _code as movement_code,
        ...
        (d ->> 'amount')::varchar as amount
        from
         jsonb_array_elements(_details::jsonb) as d;
    elsif details_count > 0 and _motion_concept_code = "02"
...
CREATE
OR REPLACE PROCEDURE "system".sp_update_access_level(IN _permissions jsonb) LANGUAGE plpgsql AS
$$
BEGIN
    perform set_config('glb.user_current',_user_code,false);
UPDATE
